...
// ── Shared helper ────────────────────────────────────────────────────────────
function CtrlBtn({ active, onClick, redOff, children }: { active: boolean; onClick: () => void; redOff?: boolean; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className={clsx(
        "w-12 h-12 rounded-full flex items-center justify-center transition-all",
        active ? "bg-white/20" : redOff ? "bg-red-500" : "bg-white/10"
      )}
    >
      {children}
    </button>
...
function CallOverlay({ type, targetName, targetAvatar, groupMembers = [], onClose }: CallOverlayProps) {
  ...
  const hangup = () => { setState("ended"); setTimeout(onClose, 700); };
    ...
    <div className="absolute inset-0 z-[100] flex flex-col select-none">
      ...
      {isVideo ? (
        <div className="absolute inset-0 bg-gray-950 flex items-center justify-center">
          {camOn
            ? <div className="text-gray-700 text-xs">Kamera bağlanıyor…</div>
      ...
      <div className="relative flex flex-col h-full text-white px-5 py-6">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5">
          ...
          {isVideo && state === "connected" && (
            <div className="absolute top-20 right-4 w-20 h-28 rounded-xl border border-white/40 bg-gray-800 shadow-xl flex items-center justify-center overflow-hidden">
...
// ── Group Chat View ───────────────────────────────────────────────────────────
function GroupChatView({ onBack, nearbyUsers }: { onBack: () => void; nearbyUsers: any[] }) {
  const { user } = useAuth();
...
// ── Private Chat View ─────────────────────────────────────────────────────────
function PrivateChatView({ selectedUser, onBack }: { selectedUser: any; onBack: () => void }) {
  const { user } = useAuth();
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    await sendMessage.mutateAsync({ receiverId: selectedUser.id, content: msgContent });
    setMsgContent("");
  };
    ...
    <div className="flex flex-col h-full bg-[#f0f2f5] overflow-hidden" style={{ userSelect: "none" }}>
      ...
      >
        ...
        <div className="flex items-center gap-0.5">
          <button onClick={() => setCall("voice")} className="p-1.5 text-primary hover:bg-gray-100 rounded-full">
            <Phone className="w-4 h-4" />
          </button>
          <button onClick={() => setCall("video")} className="p-1.5 text-primary hover:bg-gray-100 rounded-full">
            <Video className="w-4 h-4" />
          </button>
          <DropdownMenu>
  ...
  if (selectedUser) {
    return (
      <MobileContainer showNav={false}>
        <PrivateChatView selectedUser={selectedUser} onBack={() => setSelectedUser(null)} />
      </MobileContainer>
    );
  }
    ...
    <MobileContainer>
      <div className="flex flex-col bg-white min-h-full pb-16">
        ...
        <div className="bg-sky-500 px-6 pt-10 pb-4 rounded-b-[2rem] shadow-md relative overflow-hidden">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sky-100/90 text-sm">Komşularınla bağlantıda kal</p>
              </div>
            </div>
            <button
              onClick={() => setShowGroupChat(true)}
              className="flex items-center gap-1.5 bg-white/20 text-white text-xs font-bold px-3 py-1.5 rounded-full hover:bg-white/30 transition-colors shrink-0"
            >
          ...
          {/* Tabs */}
          <div className="flex gap-1 mt-3 overflow-x-auto hide-scrollbar relative z-10">
            {[
              ...
              <button
                ...
                className={clsx(
                  ...
                  activeTab === tab.id ? "bg-white text-sky-600 shadow" : "bg-white/20 text-white"
                )}
              >
                {tab.label}
              </button>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* GROUP TAB — quick card + group messages preview */}
          {activeTab === "group" && (
          ...
          {activeTab === "search" && (
            <div className="divide-y divide-gray-50">
              ...
              {(searchResults as any[]).map((u: any) => (
                <UserRow key={u.id} u={u} onClick={() => setSelectedUser(u)} sub={u.email} />
              ))}
            </div>
          )}
